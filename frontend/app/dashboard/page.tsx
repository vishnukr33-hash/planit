...
function KPICard({ label, value, icon, color, onClick }: KPIProps) {
  return (
    <div onClick={onClick} className={'card p-5 flex items-center gap-4 transition-all duration-200 ' + (onClick ? 'cursor-pointer hover:shadow-lg hover:-translate-y-0.5 hover:ring-2 hover:ring-blue-400/40' : '')}>
      ...
      {onClick && <div className="ml-auto text-slate-300 dark:text-slate-600 text-lg">›</div>}
    </div>
  )
}
...
export default function DashboardPage() {
  ...
  const trendData = {
    ...
    datasets: [{ label: 'Completed', data: trendDays.map((d) => trendMap[d] || 0), borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', fill: true, tension: 0.4 }],
  }

  const pieLabels = ['Done', 'In Progress', 'Pending', 'Need Discussion', 'Delayed']
  ...
  const statusData = { labels: pieLabels, datasets: [{ data: pieLabels.map((l) => statusMap[l] || 0), backgroundColor: pieColors, borderWidth: 2 }] }
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card p-5">
            ...
            <Doughnut data={statusData} options={{ responsive: true, plugins: { legend: { position: 'bottom' } }, onClick: handlePieClick }} />
          </div>
        </div>
            ...
            {data?.userProductivity?.length > 0 ? (
              <div className="space-y-3">
                {data.userProductivity.map((u: any) => (
                  <div key={u._id}>
                    <div className="flex justify-between text-sm mb-1"><span>{u.name}</span><span>{u.done}/{u.total} ({Math.round(u.productivity)}%)</span></div>
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min(u.productivity, 100)}%` }} /></div>
                  </div>
                ))}
              </div>
            ) : <p className="text-center py-8 text-slate-400">No productivity data yet</p>}
          </div>
        </div>
      </div>
    </DashboardLayout>
