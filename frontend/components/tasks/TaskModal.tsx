          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                )}assName="label">Category {!canEditAllFields && <span className="text-slate-400 font-normal text-xs">(read-only)</span>}</label>
                {canEditAllFields ? (
            ...
            {/* Assign To — for roles that can assign tasks */}
            {user?.role !== 'user' && (
              <div>
                <label className="label">Assign To</label>
                <select className="input" value={form.assignedTo} onChange={e => setForm(f => ({ ...f, assignedTo: e.target.value }))}>
